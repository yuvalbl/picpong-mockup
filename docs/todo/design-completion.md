...
- [x] IMPLEMENTED (A2): Service taxonomy fixed - the `projects.html` filter chips + `data-cat` tags now read **Conventions / Events / Trade show booths** (the client's Events, Conventions, Trade show booths); the old "Exhibitions" taxonomy is gone. Implemented in v5; **pending confirmation of D4 (final wording)**. Built ahead of its gating decision (D4) - see README Phase 0.
...
> Line-number caveat: this doc was written before the v5 code edits, so raw `L###` citations below have drifted (a ~47-line sustainability block was inserted in `styles.css` around line 485, pushing later CSS lines +47; `index.html` lines below the hero shifted +3..+6). Treat every `L###` as indicative only and locate code by the named selector / class / function via `grep`, not by the line number.
...
Accumulation works: `selection[]` array, toggle add/remove, thumbnail chips in the drawer, live count badge on the fab (the selection/drawer block in `app.js`). However the checklist wants "confirmation/toast on FIRST add only, not every add." Current behavior (the `.media__more` add handler, where the `t("Added — ", ...)` toast fires) is the opposite: the first add silently opens the drawer (no toast), and every subsequent add fires an "Added - N items" toast. If the intent is "one confirmation then quiet accumulation," this needs reworking. Remove-add also toasts `t("Removed", ...)`.
...
`.contact-fab` is in the static markup of all six pages, fixed bottom-right with a live count badge (the `.contact-fab` / `.contact-fab__badge` rules in `styles.css`). On mobile (the `max-width:520px` query) it only tightens padding and keeps the full "Request a quote" text label - it does not shrink to an icon, and no bottom scroll-padding is reserved on `<body>`, so the pill floats over page-bottom content. The lead drawer goes full-width on mobile (`width:100vw` under the `max-width:560px` query), which is fine. Net: functional, but the always-expanded pill over unpadded content is exactly the "obscures content on mobile" edge to tighten.
...
**12. Loud/primary sustainability section (nature imagery, hi-tech CSR target) - IMPLEMENTED in v5 (A1); pending D6 + real photo.**
This was the biggest content/design gap at audit time (then a quiet two-column split), and it has now been built out in v5 as a loud, primary sustainability section (the `panel--pond` -> `--forest` ocean-to-canopy gradient block on the home page, with CSR-targeted framing). Remaining: the section uses a placeholder nature image (real ocean/rainforest photo to be supplied), and its final intensity is gated on D6. The audit's original "certification documents to be attached before publication" caveat still applies. Status: implemented, pending confirmation of D6 + the real nature-photo asset.

**13. English service taxonomy - IMPLEMENTED in v5 (A2); pending D4 wording confirmation.**
Primary nav is section-based (Catalog / Projects / Latest / Sustainability / About / Contact), not a service taxonomy. The projects filter chips now read "All work / Conventions / Events / Trade show booths" and each project's `data-cat` is `conventions | events | booths` (verified: zero "Exhibition" remains in the chips, tags, or filter logic). This matches the client's "Events, Conventions, Trade show booths." Catalog facets are product-type categories (Counters / Stands / Shelving / Photocall walls / Gates & arches), a separate axis and fine. Status: implemented, pending confirmation of D4 (final wording). Note: "exhibition" still appears as a plain common noun in some body copy (e.g. "exhibition stands"), which is prose, not the taxonomy, and out of scope for A2.
...
**Bucket 2 - Design gaps.** A1 (implemented in v5, pending D6 + real nature photo): a loud, CSR-manager-targeted sustainability section with nature imagery, replacing the old quiet split. A2 (implemented in v5, pending D4): service taxonomy fixed so filters/tags read Conventions, Events, Trade show booths (old "Exhibitions" gone). Still deferred: a dedicated products eco module (Recycle / Resistance pillars) rather than scattered badges.
