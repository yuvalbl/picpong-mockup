...
> ⚠️ **PARTIALLY SUPERSEDED (2026-07).** Dated 2026-05-31, pre-pivot. Still authoritative on the *look* (warm-paper canvas, pufferfish, four ownable moves, orange `#E67A2F`), but three things are overtaken: (1) **§8 "Commerce-on-calm"** (cart/checkout/account) — commerce deferred to Phase 2, not in v5; (2) **§4 type is superseded** — display face is now **`Rubik`** (decision 2026-07-05, `Fraunces` dropped); §4's Fraunces/Assistant spec is historical. Hebrew **body** still needs a Hebrew-glyph face (open a11y gap); (3) the fixed px **type scale** is superseded by `../todo/font-sizes.md`. See [`../planning-consistency-audit.md`](../planning-consistency-audit.md).
>
