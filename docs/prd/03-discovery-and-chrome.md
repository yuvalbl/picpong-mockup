...
- **Live, type-to-filter** (`input` event, debounced) over the **rendered project tiles**; no submit/navigation. Indexed text per tile: **title (H3/H4) + H2/subtitle + tag + client** (and any `data-search-text` we add). Match is **case- and diacritic-insensitive, substring** — "diacritic-insensitive" means **Hebrew niqqud** (the vowel-point marks, e.g. דּוּכָן) and **Latin accents** (café) are stripped from both the query and the indexed text before comparing, so vowel-pointing never blocks a match. Indexing reads the **active language only** (the rendered `textContent` after the language swap).
